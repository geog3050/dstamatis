...
######################################################################
def hawkid():
    return(["Dionysios Stamatis", "dstamatis"])
...
def problem1():
    mode = input("Enter the calculation method (min, max, avg): ") 
    count = int(input("Enter the number of values: ")) 
    values = [] # give values like 1 and 2

    for i in range(count):
        value = float(input(f"Enter value {i+1}: "))
        values.append(value)

    print(info[0])  # Print name
    print(info[1])  # Print hawkid
    print(mode)  # Print mode
    print(count)  # Print count
    print(len(values))  # Print length of the list
    print(values)  # Print the list
    
problem1()  # Call the function to execute
...
def problem1():
    ...
    print(len(values))  # Print length of the list
    print(values)  # Print the list
    problem2(mode, values)  # Call problem2 with 'mode' and 'values'
    
problem1()  # Call the function to execute

def problem2(mode, values):
    if mode == 'min':
        minimum = values[0]
        for value in values:
            if value < minimum:
